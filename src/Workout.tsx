...
import { GOAL_LABELS, MAX_SETS, MUSCLE_LABELS, estimateExerciseSeconds, estimatePlanSeconds, getExercise, getSubstitutions, intensityOptions, isBodyweightExercise, isFocusExercise, minimumRestSeconds, planBudgetNote, supersetCandidates, validatePlan, workoutSetSteps } from './domain'
...
function SetRow({ step, parent, log, onLog, onUndo, blocked, locked }: {
  step: SetStep; parent?: SetLog; log?: SetLog; onLog: (log: SetLog) => void; onUndo: (id: string) => void; blocked: boolean; locked: boolean
}) {
  ...
  return <form className={`set-row ${log ? 'set-complete' : ''} ${part ? 'mini-set' : ''}`} data-part={part ?? 'regular'} aria-label={label} onSubmit={(event) => {
    ...
    <input aria-label={`${label} carico kg`} inputMode="decimal" type="number" min="0" max="1000" step="0.25" placeholder="--" value={log ? log.weight ?? '' : currentWeight} disabled={Boolean(log) || blocked || locked} onChange={(event) => setWeight(event.target.value)} />
    ...
    <input aria-label={`${label} RIR`} inputMode="numeric" type="number" min="0" max="10" step="1" placeholder="--" value={log ? log.rir ?? '' : rir} disabled={Boolean(log) || blocked || locked} onChange={(event) => setRir(event.target.value)} />
        ...
        <div className="set-table"><div className="set-table-head"><span>Serie</span><span>Kg</span><span>Rip.</span><span>RIR</span><span className="sr-only">Completa</span></div>
          {blockSteps.map((step) => {
            ...
            return <div key={`${step.item.id}-${step.setIndex}-${step.part ?? 'regular'}`}>
              ...
              <SetRow step={step} parent={parent} log={log} onLog={onLog} onUndo={onUndo} blocked={blocked}
